...
app.get('/flix', (req, res) => {
  ...
  // Query to fetch movies along with their liked status for the current user
  const query = `
    SELECT 
      movies.movie_id, 
      movies.name, 
      ...
      movies.director, 
      movies.youtube_link,
      CASE WHEN user_to_movie_liked.user_id IS NULL THEN FALSE ELSE TRUE END AS liked
    FROM movies
    LEFT JOIN user_to_movie_liked 
    ON movies.movie_id = user_to_movie_liked.movie_id 
    AND user_to_movie_liked.user_id = $1`;

  db.any(query, [username])
    .then(data => {
      // Render the flix page with movie data
...
app.post('/likeMovie', (req, res) => {
  ...
  if (referer.includes('/movieDetails')) {
    // If the like action was performed from the movie details page, redirect back to that page
    res.redirect(`/movieDetails?id=${movieID}`);
  } else {
    res.redirect(`/flix`);
  }
